(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('timeManagerApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-timeManagerApp-error');
                var entityKey = httpResponse.headers('X-timeManagerApp-params');
                if (errorHeader) {
